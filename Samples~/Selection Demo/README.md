...
Click a world object to select it through ObjectSelection. The ObjectSelection-CoreState bridge updates CoreState, and the UIBinding-CoreState bridge updates the UI list selected key. The UI list uses sample-only row tinting for selected and normal rows.
